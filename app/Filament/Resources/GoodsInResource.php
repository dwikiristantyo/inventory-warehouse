...
use App\Filament\Resources\GoodsInResource\Pages;
use App\Models\Item;
...
class GoodsInResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                Components\Hidden::make('trans_type')->default('IN'),
                Components\TextInput::make('trans_no')
                    ->default('IN-' . date('YmdHis'))
                    ->disabled()
                    ->dehydrated()
                    ->required(),
                    ...
                    ->schema([
                        Components\Select::make('item_code')
                            ...
                            ->afterStateUpdated(function ($state, Set $set) {
                                ...
                                // Ambil uom1 dan uom2 berdasarkan item yang dipilih
    ...
    {
        return $table
            ->query(TransactionHeader::query()->where('trans_type', 'IN')->where('status', '!=', 'X'))
    ...
    {
        return [
            'index' => Pages\ListGoodsIns::route('/'),
            'create' => Pages\CreateGoodsIn::route('/create'),
            'view' => Pages\ViewGoodsIn::route('/{record}'),
